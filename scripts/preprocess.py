...
# med_count_cols = [col for col in medication_columns if col in data.columns]
# data['total_medications'] = data[med_count_cols].sum(axis=1)
# integer.append('total_medications')
...
data['diagnosis_group'] = data['diag_1'].apply(categorize_diagnosis)

# admission_type_id into separate clinically meaningful categories
...
data['admission_type_group'] = data['admission_type_id'].apply(
...
# discharge_disposition_id into clinically meaningful categories
...
data['discharge_disposition_group'] = data['discharge_disposition_id'].apply(
    lambda x: 'Home' if x in home_discharge else
              'Facility' if x in facility_discharge else
              'Expired' if x in expired else 'Other')

# admission_source_id into clinically meaningful categories
...
data['admission_source_group'] = data['admission_source_id'].apply(
...
selected_features = [f for f in selected_features if f not in ['admission_type_id', 'discharge_disposition_id', 'admission_source_id', 'diag_1']]
selected_features.extend(['admission_type_group', 'discharge_disposition_group', 'admission_source_group', 'diagnosis_group'])
...
non_numeric_cols = selected_data.select_dtypes(exclude=['number']).columns.tolist()
if non_numeric_cols:
    ...
    for col in non_numeric_cols:
        if col != target_column:
            try:
                ...
                if len(unique_values) <= 2 and all(x in [0, 1, '0', '1', True, False, np.nan, None] for x in unique_values):
                    selected_data[col] = selected_data[col].astype(int)
                else:
                    selected_data[col] = selected_data[col].astype(float)
            except Exception as e:
                print(f"Error converting {col}: {e}")
                selected_data[col] = pd.to_numeric(selected_data[col], errors='coerce')

                # fill NaN values
                if selected_data[col].isna().any():
                    if len(selected_data[col].unique()) <= 2:  # Binary column
                        selected_data[col] = selected_data[col].fillna(0).astype(int)
                    else:  # non-binary column
                        selected_data[col] = selected_data[col].fillna(selected_data[col].median())
